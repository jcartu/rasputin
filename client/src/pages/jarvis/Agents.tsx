...
import { Brain, Code, Terminal, Shield, Eye, Activity, Layers } from "lucide-react";
...
export default function Agents() {
  const agents = [
    ...
    },
    {
      ...
      tools: ["Task Decomposition", "Dependency Mapping", "Resource Allocation"]
    },
    {
      name: "Coder",
      icon: Code,
      ...
      tools: ["File Operations", "LSP Integration", "Git Control", "Test Runner"]
    },
    {
      name: "Executor",
      icon: Terminal,
    ...
    <Layout>
      <div className="space-y-10">
        <div className="space-y-4">
          <h1 className="text-3xl font-bold tracking-tight font-mono">3. Swarm Agent Specification</h1>
            ...
            <Card key={i} className="bg-card/50 border-border hover:border-primary/50 transition-colors">
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="outline" className="font-mono text-xs">{agent.role}</Badge>
                </div>
                <CardTitle className={`text-xl font-mono mt-4 ${agent.color}`}>{agent.name}</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="text-xs font-mono uppercase text-muted-foreground">Key Tools</div>
                  <div className="flex flex-wrap gap-2">
                    {agent.tools.map((tool, j) => (
                      <Badge key={j} variant="secondary" className="text-xs bg-muted/50">
                        {tool}
                      </Badge>
                    ))}
                  </div>
          ...
          <Card className="bg-black/30 border-border">
            <CardContent className="p-6 font-mono text-sm space-y-4">
              <div className="flex items-center gap-4">
                <Badge className="bg-purple-500/20 text-purple-400 hover:bg-purple-500/30">Redis Streams</Badge>
                <span>Primary communication bus. All events are immutable and ordered.</span>
              </div>
              <div className="flex items-center gap-4">
                <Badge className="bg-blue-500/20 text-blue-400 hover:bg-blue-500/30">Consumer Groups</Badge>
                <span>Agents subscribe to relevant topics (e.g., `task.created`, `code.written`).</span>
              </div>
              <div className="flex items-center gap-4">
                <Badge className="bg-green-500/20 text-green-400 hover:bg-green-500/30">Anti-Thrash</Badge>
                <span>Distributed locking prevents multiple agents from controlling the mouse/keyboard simultaneously.</span>
              </div>
            </CardContent>
          </Card>
        </section>
