...
import { Brain, ChevronDown, ChevronRight, CheckCircle2, Loader2, Terminal, Shield, Zap, Code, Activity } from 'lucide-react';
...
export const IntelligenceStream: React.FC<IntelligenceStreamProps> = ({ scenario, onComplete, onLog }) => {
  ...
  const updateMessageContent = (id: string, content: string) => {
    setMessages(prev => prev.map(m => 
      m.id === id ? { ...m, content, isThinking: false } : m
    ));
  ...
  const simulateReasoning = async (msgId: string, steps: string[]) => {
    for (const stepText of steps) {
      setMessages(prev => prev.map(m => {
        ...
        const updatedReasoning = (m.reasoning || []).map(s => ({ ...s, status: 'complete' as const }));
    ...
    setMessages(prev => prev.map(m => {
      if (m.id !== msgId) return m;
      return { ...m, reasoning: (m.reasoning || []).map(s => ({ ...s, status: 'complete' as const })) };
    }));
  };
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto p-6 space-y-6 font-mono text-sm">
      {messages.map((msg) => (
        ...
        >
          <div className={`max-w-[90%] w-full ${msg.role === 'user' ? 'bg-primary/10 border border-primary/30' : 'bg-card/50 border border-border'} rounded-lg p-4 shadow-lg backdrop-blur-sm overflow-hidden`}>
              ...
              <div className="space-y-4">
                ...
                {msg.reasoning && msg.reasoning.length > 0 && (
                  <div className="border-l-2 border-primary/30 pl-4 my-2">
                    ...
                    >
                      {msg.isThinking ? <Loader2 className="w-3 h-3 animate-spin" /> : <Activity className="w-3 h-3" />}
                      <span className="uppercase tracking-wider font-bold">
                        {msg.isThinking ? 'EXECUTION PROTOCOL ACTIVE' : 'OPERATION LOG'}
                      </span>
...
const getStepType = (text: string) => {
  if (text.includes('Executing') || text.includes('Running') || text.includes('Compiling')) return 'exec';
  if (text.includes('Scanning') || text.includes('Connecting') || text.includes('Fetching')) return 'net';
  if (text.includes('Querying') || text.includes('Retrieving') || text.includes('Saving')) return 'db';
  return 'default';
};
...
const getAgentForStep = (text: string) => {
  ...
  if (text.includes('Analyzing') || text.includes('Calculating')) return 'PLANNER';
  return 'SYSTEM';
};
...
const getInitialQuery = (scenario: string) => {
  switch (scenario) {
    case 'security': return "Initiate global cyber-defense audit. Scan all subnets for anomalies and patch vulnerabilities.";
    case 'saas': return "Deploy a full-stack SaaS for 'CryptoTracker'. Stack: React, Node, Postgres. Deploy to Vercel.";
    case 'market': return "Execute high-frequency arbitrage across Binance and Coinbase. Target: BTC/USD spread > 0.5%.";
    case 'bugfix': return "Analyze screen buffer. Identify the infinite loop in the active Python script and apply fix.";
...
const getFollowUpQuery = (scenario: string) => {
  switch (scenario) {
    case 'security': return "Isolate the compromised nodes in Subnet 4 and deploy honeypots.";
    case 'saas': return "Run integration tests and verify SSL certificate propagation.";
    case 'market': return "Increase leverage to 5x and rebalance portfolio for delta neutrality.";
    case 'bugfix': return "Verify the fix with a dry run.";
