...
import { useDocsTheme, type DocsTheme } from "@/contexts/DocsThemeContext";
import { playDocsSound } from "@/lib/docsSound";

const themes: { id: DocsTheme; name: string; color: string }[] = [
...
export default function DocsThemeSelector() {
  const { theme, setTheme } = useDocsTheme();

  const handleThemeChange = (newTheme: DocsTheme) => {
    playDocsSound("hover", newTheme);
    setTheme(newTheme);
  };
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          variant="outline"
          size="icon"
          className="relative border-primary/20 hover:border-primary/50 hover:bg-primary/10"
        >
      ...
      <DropdownMenuContent
        align="end"
        className="w-56 bg-card/95 backdrop-blur border-primary/20"
      >
