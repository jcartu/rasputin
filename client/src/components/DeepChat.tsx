...
import { Brain, ChevronDown, ChevronRight, CheckCircle2, Loader2, MessageSquare, Terminal } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto p-6 space-y-6 font-mono text-sm">
      {messages.map((msg) => (
        ...
        >
          <div className={`max-w-[80%] ${msg.role === 'user' ? 'bg-primary/10 border border-primary/30' : 'bg-card border border-border'} rounded-lg p-4 shadow-lg backdrop-blur-sm`}>
              ...
              <div className="space-y-4">
                ...
                {msg.reasoning && msg.reasoning.length > 0 && (
                  <div className="border-l-2 border-primary/30 pl-4 my-2">
                    ...
                    >
                      {msg.isThinking ? <Loader2 className="w-3 h-3 animate-spin" /> : <Brain className="w-3 h-3" />}
                      <span className="uppercase tracking-wider font-bold">
                        {msg.isThinking ? 'Thinking Process Active' : 'Reasoning Chain'}
                      </span>
...
const getInitialQuery = (scenario: string) => {
  switch (scenario) {
    case 'medical': return "Patient presents with recurring fever, joint pain, and rash after travel to Brazil. Initial tests for Dengue are negative. Analyze potential rare tropical pathogens.";
    case 'legal': return "Analyze the precedent set by the 'New York Times v. OpenAI' case regarding fair use in AI training data. How does this impact current copyright law in the EU?";
    case 'market': return "What is the consensus on the impact of TSMC's Arizona plant delays on the global GPU supply chain for Q3 2025?";
    case 'code': return "Audit this smart contract for reentrancy vulnerabilities. It uses a checks-effects-interactions pattern but interacts with untrusted external tokens.";
    default: return "Initiate deep research protocol.";
  }
};

const getFollowUpQuery = (scenario: string) => {
  switch (scenario) {
    case 'medical': return "Could it be Chikungunya or Zika? Cross-reference symptoms with recent outbreaks in the region.";
    case 'legal': return "Does the EU AI Act provide any specific exemptions for research organizations in this context?";
    case 'market': return "Factor in the potential new export restrictions from the US Department of Commerce.";
    case 'code': return "What if the external token implements a malicious transfer hook? Generate a test case.";
    default: return "Elaborate on that point.";
  }
};
